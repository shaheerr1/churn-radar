...
function OverviewTab({
  ...
  const shapColor = (imp: number) =>
    imp / maxImp > 0.7 ? "#ff3b3b" : imp / maxImp > 0.4 ? "#ff9500" : "#0af";
                ...
                >
                  ...
                  {top && (
                    <div className="db-pq-reason">
                      ↑ {top.feature} (+{top.shap_impact.toFixed(2)}) ·{" "}
                      {c.contract} · {Math.round(c.tenure)}mo
                    </div>
                  )}
                  {c.recommendation && (
...
function PredictTab() {
  ...
  const run = async () => {
    ...
    try {
      ...
      setError(
        `Error: ${e instanceof Error ? e.message : "could not reach /predict"}`,
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="db-panel">
        ...
        {loading && (
          <div className="db-empty">
            ...
            />
            <div className="db-skel" style={{ width: 80, margin: "0 auto" }} />
          </div>
        )}
      ...
      <div className="db">
        ...
        <div className="db-hdr">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span className="db-stream-badge" style={streamStyle}>
              {streamStatus === "live"
                ? "● stream live"
                : streamStatus === "connecting"
                  ? "◌ connecting…"
